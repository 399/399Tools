...
export default async function CityGrid() {
    ...
    if (!isConfigured) {
        return (
            <div className="col-span-full p-8 border-2 border-dashed border-red-200 rounded-[32px] bg-red-50 text-center">
                <p className="text-red-600 font-bold mb-2">⚠️ 飞书配置缺失</p>
                <p className="text-red-500 text-sm">请在根目录 .env 文件中配置 FEISHU_ 相关环境变量后重新启动。</p>
            </div>
        );
    }
    ...
    const cities: CityMap[] = Object.entries(cityGroups).map(([city, count]) => {
        ...
        return {
            ...
            image: imagePath,
            link: `/9map/map?city=${encodeURIComponent(city)}`,
            count,
            skyColor,
        };
    });
